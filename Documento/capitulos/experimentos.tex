...
El experimento se estructura en una serie de diez ejecuciones de envío de tráfico, realizadas primero con el filtro XDP desactivado y posteriormente con dicho filtro activado. La finalidad de cada conjunto de pruebas depende de la herramienta utilizada: con Iperf, el objetivo es evaluar si el filtro introduce un posible cuello de botella en un entorno idealizado, caracterizado por tráfico homogéneo TCP, que es el protocolo más común. En el caso de TCPReplay, el experimento permite analizar si el filtro genera cuellos de botella diferenciados para tráfico benigno y tráfico malicioso por separado. Finalmente, utilizando el script desarrollado específicamente para este trabajo, se pretende medir el impacto del filtro en un escenario de ataque en el que tanto el tráfico benigno como el malicioso son elevados. En todos los experimentos se varían los bits por segundo (bps) o los paquetes por segundo (PPS) con el fin de evaluar el rendimiento bajo diferentes cargas. Adicionalmente, en las pruebas con Iperf se modifican los tamaños de los paquetes, mientras que con TCPReplay y el script propio esto no es posible debido a que los paquetes se relanzan directamente desde capturas previamente almacenadas.
